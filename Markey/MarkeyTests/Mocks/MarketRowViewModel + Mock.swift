...
extension MarketRowViewModel {
    convenience init(stockName: String, lastPrice: String, changePercent: String) {
        let marketPrice = MarketPrice(stockName: stockName, lastPrice: lastPrice, changePercent: changePercent)
        self.init(marketPrice: marketPrice)
    }

    static func mock(stockName: String = "", lastPrice: String = "", changePercent: String = "") -> MarketRowViewModel {
        let marketPrice = MarketPrice(stockName: stockName, lastPrice: lastPrice, changePercent: changePercent)
        return MarketRowViewModel(marketPrice: marketPrice)
    }
}
